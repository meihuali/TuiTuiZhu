...
import com.designer.library.base.LibraryKt;
...
public class TabLayout extends HorizontalScrollView {
     ...
     *         Value from [0, 1) indicating the offset from {@code position}.
     ...
     * </ul>
     * @param mode
     *         one of {@link #MODE_FIXED} or {@link #MODE_SCROLLABLE}.
     */
     ...
     * @param viewPager
     *         the ViewPager to link to, or {@code null} to clear any previous link
     */
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                ...
                final View selectedTitle = getChildAt(mSelectedPosition);
                int paddingLeft = 0;
                if (selectedTitle != null && selectedTitle instanceof TabView) {
                    TabView tabView = (TabView) selectedTitle;
                    TextView textView = tabView.mTextView;
                    if (textView != null && !TextUtils.isEmpty(textView.getText())) {
                        paddingLeft = (selectedTitle.getWidth() - textView.getWidth()) / 2;
                    }
                }

                //这里设置文字位置
                @SuppressLint("DrawAllocation")
                RectF oval3 = new RectF(mIndicatorLeft + paddingLeft, getHeight() - mSelectedIndicatorHeight,
                        mIndicatorLeft + paddingLeft + mTabLineOffset, getHeight());
                canvas.drawRoundRect(oval3, DimensionsKt.dip(LibraryKt.getApp(), 3), DimensionsKt.dip(LibraryKt.getApp(), 3),
       ...
       public void draw(Canvas canvas) {
           ...
           if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
               if (mTabLineOffset == 0) {
                   ...
                   //这里设置文字位置
                   RectF oval3 = new RectF(tabCenter - mTabLineOffset, getHeight() - mSelectedIndicatorHeight, mIndicatorRight - width / 2 + mTabLineOffset, getHeight());
    ...
    private class AdapterChangeListener implements ViewPager.OnAdapterChangeListener {
        ...
        public void onAdapterChanged(@NonNull ViewPager viewPager,
                                     @Nullable PagerAdapter oldAdapter, @Nullable PagerAdapter
                                             newAdapter) {
