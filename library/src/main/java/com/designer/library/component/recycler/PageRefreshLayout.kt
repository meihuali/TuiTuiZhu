...
open class PageRefreshLayout : SmartRefreshLayout, OnMultiPurposeListener {
    ...
    override fun setRefreshContent(content: View): RefreshLayout {
        ...
        if (mHandler != null) {
            val fixedHeaderView = if (mFixedHeaderViewId > 0) thisView.findViewById<View>(mFixedHeaderViewId) else null
            val fixedFooterView = if (mFixedFooterViewId > 0) thisView.findViewById<View>(mFixedFooterViewId) else null
