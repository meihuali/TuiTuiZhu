...
public class Linkify {
        ...
        public final boolean acceptMatch(CharSequence s, int start, int end) {
            try {
                ...
                try {
                    ...
                    if (nextChar < 256 && !((0 <= UrlEndAppendNextChars.indexOf(nextChar)) || Character
     ...
     */
    public static boolean addLinks(Spannable text, int mask, ColorStateList linkColor, ColorStateList bgColor, OnSpanClickListener l) {
        if (mask == 0) {
            return false;
        }
...
//        PhoneNumberUtil phoneUtil = PhoneNumberUtil.getDefault();
