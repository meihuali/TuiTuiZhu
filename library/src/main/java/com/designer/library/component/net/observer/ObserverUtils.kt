 ...
 */
fun <M> Observable<M>.net(lifecycleOwner: LifecycleOwner? = null, block: (NetObserver<M>.(M) -> Unit)? = null) {
 ...
 * @param block (M) -> UnitUtils
 */
fun <M> Observable<M>.refresh(smartRefreshLayout: SmartRefreshLayout, block: RefreshObserver<M>.(M) -> Unit) {
