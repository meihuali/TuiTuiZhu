...
package com.designer.library.component.swipeback
...
@JvmOverloads constructor(
    ...
    fun startAnimator(startValue: Float, minFinalValue: Float, maxFinalValue: Float, velocity: Float) {
        ...
        if (finalValue < minFinalValue || finalValue > maxFinalValue) {//终点值在界外
            ...
            //校正终点值，计算位移距离和动画时间
            mFinalValue = if (finalValue * 2 < minFinalValue + maxFinalValue) minFinalValue else maxFinalValue
            mDistance = mFinalValue - startValue
            mDuration = getDurationByDistance(mDistance)
        }
        startAnimator()
    ...
    fun getDurationByDistance(distance: Float, frictionCoeff: Float = 1f): Long {
        var duration: Long = 0
        if (distance != 0f) {
            val base = (Math.abs(distance) / (mFlingFriction * frictionCoeff * mPhysicalCoeff)).toDouble()
    ...
    private inner class DecelerateEvaluator : TypeEvaluator<Float> {
        ...
        override fun evaluate(fraction: Float, startValue: Float?, endValue: Float?): Float? {
            ...
            if (!isBouncing) {//禁用回弹效果（可能越界，需要提前结束动画）
                ...
                if (fraction <= 1f - bounceFraction) {//第一阶段，按原本位移距离和动画时间进行计算
                    ...
                    val distance = getDistance(fraction, mBounceDuration / 2, mBounceDistance, mFrictionCoeff)
         ...
         * @param frictionCoeff 摩擦系数
         */
        private fun getDistance(fraction: Float, duration: Long, distance: Float, frictionCoeff: Float): Float {
            //获取剩余动画时间
            val surplusDuration = ((1f - fraction) * duration).toLong()
            //计算剩余位移距离
            val surplusDistance = getDistanceByDuration(surplusDuration, frictionCoeff) * Math.signum(distance)
            //计算位移距离
